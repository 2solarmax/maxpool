...
**Status:** re-integrated onto current main + RE-ASSESSED (2026-06-27) — branch `feat/reload-reintegrate` (clean merge of #3+#2+#1; no conflicts; removed-import refs gone). **The "INCOMPLETE / 6 must-fixes outstanding" label was STALE.** Run per-file in isolation the product code is effectively done and the **brick-safety is empirically PROVEN**:
- `reload-protocol` 5/5, `reload-generation-guard` 3/3 (config-write barrier + stateGeneration), `reload-tty-single-owner` 2/2, `reload-oauth-refresh-across-reload` 1/1, **`reload-refresh-torture` 3/3 (concurrent-refresh → no double-spend / invalid_grant — THE brick-safety proof)**, and `B2` (supervisor-stop-own-accept-loop) all PASS in isolation.
- The 6 must-fixes are present in `src/index.js`: supervisor accept-loop stop after MSG_PRIMARY (~:218), `setWriterLease(false)` at boot (:446), stateGeneration gen-guard (:455), supervisor `uncaughtException`/`unhandledRejection` (:158), exit-75 fallback.
**Real remaining blocker = TEST FLAKINESS under contention, NOT product correctness.** The 6 worker-spawning integration files contend on ports/CPU when run together (`node --test` runs files concurrently; `B2` fires 200 concurrent requests → got 52/200 with fast ECONNRESET in the full-file run, 200/200 alone). Finish = make the integration tests robust (serialize the reload files / per-test child-kill + port-release waits / soften the steady-state assertion) → full suite reliably green → final focused review (brick-safety already proven by the torture test, so a single-agent review suffices) → merge + release. Today's 2s self-healing restart blip is benign, so #6 stays an optimization, not a fix.
