...
**Status:** DEEP-DIVE (2026-06-27, branch `feat/reload-reintegrate`). Product CORE works + brick-safety PROVEN (`reload-refresh-torture` 3/3 — concurrent-refresh, no token double-spend). The reload SUITE was made reliable: B2 asserts the no-parent-swallow invariant (not a brittle 200-burst count); files run one-per-process (run-tests.sh) so subprocess tests don't contend; the child exit-wait is guarded (was hanging the suite); **and a real M4 test bug was fixed — it sent SIGUSR2, which maxpool doesn't handle, so Node's default TERMINATED the supervisor and the reload never cut over.** After that fix M4 cuts over both reloads.
**Single residual before ship:** `reload-state-generation.test.js` M4 line 118 — the on-disk state `_generation` does not advance within 20s after the 2nd reload. The quota-save interval is a HARDCODED 60s (`index.js:595`; M4's "short interval" comment is wrong), so M4 relies on the reload RELEASE-flush to bump the generation. Needs a focused product-vs-test call: either the reload release-flush doesn't reliably advance `_generation` (a real, NON-bricking persistence bug to fix in `index.js` MSG_RELEASE) OR M4 is over-strict (force a quota change / lengthen the wait / set a real short save interval). Everything else in the reload suite is green per-file. NOT shipped until this is resolved + a focused review.
