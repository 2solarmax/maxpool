...
| grep --line-buffered -E "fetch failed|all routes failed|No route for request|connection_unavailable|Network soak budget|queueing request|^[0-9-]{10}T[0-9:]{8}Z (FAIL|SLOW|DNSFAIL|DNSSLOW)|rc_gate_upstream_error|direct-error.*v1/messages|loop-stall|\[conn\] accepts" \
